...
export class GalaxyGenerator {
    ...
    guiManager = () => {
        ...
        this.gui.add(this.parameters, 'animationSpeed').min(0.000004).max(0.00001).step(0.000001).onFinishChange(this.generateGalaxy);
    }
    

    setCameraPos = () => {
        this.camera.position.set(0,4,4)
        this.camera.lookAt(new THREE.Vector3(this.points?.position.x,this.points?.position.y,this.points?.position.z))
    }
    ...
    generateGalaxy = () => {
        ...
        for(let i = 0; i < this.parameters.count; i++) {
            ...
            starSizes[i] = Math.random() * (0.001 - this.parameters.size) + this.parameters.size;
    ...
    start = () => {
        ...
        eachFrame(this.scene, this.camera, this.renderer, (elapsedTime: number) => {
            if(this.points) {
                this.points.rotation.y += Math.PI * elapsedTime * this.parameters.animationSpeed;
            }
        }, this.controls)
    }
}